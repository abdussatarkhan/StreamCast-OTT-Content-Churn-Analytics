...
**Global SVOD Streaming Content Intelligence Data Warehouse: Binge completion rates, 90-day churn hazard models, Video Startup Time (VST) QoE, and licensing content ROI.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/StreamCast-OTT-Content-Churn-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/StreamCast-OTT-Content-Churn-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/StreamCast-OTT-Content-Churn-Analytics)
...
**StreamCast-OTT-Content-Churn-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **OTT Media & Streaming Intelligence**.
...
### 1️⃣ Executive KPI Command Center (Obsidian Cinema Red (Deep Dark))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_subscribers  │──►│     fact_playback_sessions │◄──┤     dim_titles_catalog │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_devices_ott  │   │     dim_plans_sub       │   │     dim_date         │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM streamcast_dw.fact_playback_sessions f
JOIN streamcast_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/StreamCast-OTT-Content-Churn-Analytics.git
cd StreamCast-OTT-Content-Churn-Analytics
